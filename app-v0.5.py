...
with st.sidebar:
    ...
    furniture_type = st.selectbox(
        ...
        index=0,
        key="furniture_type",
        help="Choose the type of furniture to visualize or select None for a random interior."
    )
    ...
    material = st.selectbox(
        ...
        index=0,
        key="material",
        help="Choose the material of the furniture or select None to skip."
    )
    color = st.selectbox(
        "Select Color",
        ...
        index=0,
        key="color",
        help="Choose the color of the furniture or select None to skip."
    )
    ...
    # Инициализация состояния для quality_enabled и negative_enabled
...
def generate_prompt(model_type="Fooocus"):
    if st.session_state.furniture_type == "Table" and (st.session_state.furniture_subtype == "Bedside Table" or st.session_state.furniture_subtype == "Side Table"):
        if model_type == "Flux":
            main_prompt = f"A stylish {st.session_state.furniture_subtype} as the main focus, made of {st.session_state.material} in {st.session_state.color}, featuring sturdy construction, smooth surface, modern design, and functional decor. "
            main_prompt += f"It is positioned in the center with a blurred {st.session_state.style} {st.session_state.room_type} background " if st.session_state.room_type != "None" and st.session_state.style != "None" else ""
            main_prompt += f"with {lighting_control[st.session_state.lighting]} " if st.session_state.lighting != "None" else ""
            main_prompt += f"Captured in a {camera_control[st.session_state.camera]} view focusing solely on the {st.session_state.furniture_subtype}, with the background out of focus. " if st.session_state.camera != "None" else ""
            main_prompt += "Hyperrealistic 3D rendering, ultra-detailed, 8K resolution, photorealistic, cinematic quality, sharp focus, lifelike lighting, realistic shadows, intricate details, crystal-clear image quality, bright lighting, vivid colors."
        else:  # Fooocus по умолчанию
            main_prompt = f"A stylish {st.session_state.furniture_subtype} as the main focus, made of {st.session_state.material} in {st.session_state.color}, featuring sturdy construction, smooth surface, modern design, and functional decor. "
            main_prompt += f"It is positioned in the center of a {st.session_state.style} {st.session_state.room_type} " if st.session_state.room_type != "None" and st.session_state.style != "None" else ""
            main_prompt += f"with {lighting_control[st.session_state.lighting]} " if st.session_state.lighting != "None" else ""
            main_prompt += "The room has a warm and inviting atmosphere with minimal wooden elements in the blurred background. "
            main_prompt += f"Captured in a {camera_control[st.session_state.camera]} view focusing solely on the {st.session_state.furniture_subtype}, with the background out of focus. " if st.session_state.camera != "None" else ""
            main_prompt += "Hyperrealistic 3D rendering, ultra-detailed, 8K resolution, photorealistic, cinematic quality, sharp focus, lifelike lighting, realistic shadows, intricate details, crystal-clear image quality, bright lighting, vivid colors."
    else:
        main_prompt = "Create an interior design photo" if st.session_state.furniture_type == "None" else "Create a stylish "
        main_prompt += st.session_state.room_type if st.session_state.room_type != "None" else ""
        main_prompt += " " if st.session_state.room_type != "None" else ""
        main_prompt += style_prompts[st.session_state.style] if st.session_state.style != "None" else ""
        main_prompt += " with a " if st.session_state.furniture_subtype != "None" else " with "
        main_prompt += st.session_state.furniture_subtype if st.session_state.furniture_subtype != "None" else ""
        main_prompt += " as the main focus" if st.session_state.furniture_subtype != "None" else ""
        main_prompt += ", upholstered in " + st.session_state.material if st.session_state.material != "None" else ""
        main_prompt += " and " + st.session_state.color if st.session_state.color != "None" and st.session_state.material != "None" else " and " + st.session_state.color if st.session_state.color != "None" else ""
        main_prompt += "." if (st.session_state.material != "None" or st.session_state.color != "None") else "."
        main_prompt += " The " + st.session_state.furniture_subtype if st.session_state.furniture_subtype != "None" else " furniture"
        main_prompt += " is " + subject_subcategories[st.session_state.furniture_type]["tokens"] if st.session_state.furniture_subtype != "None" else ", featuring a modern design"
        main_prompt += " and " + camera_control[st.session_state.camera] + "." if st.session_state.camera != "None" else "."
        main_prompt += " The room is " + lighting_control[st.session_state.lighting] if st.session_state.lighting != "None" else ""
        main_prompt += ", with " + position_control[st.session_state.position] if st.session_state.position != "None" else ""
        main_prompt += ", creating a balanced and inviting atmosphere."

    # Добавляем токены качества, если кнопка включена
...
with col1:
    if st.button("Optimize", key="optimize_btn", help="Optimize the prompt to fit within 400 characters"):
        main_prompt = optimize_prompt(st.session_state.main_prompt)
...
def optimize_prompt(prompt, max_length=400):
    ...
    for word in words:
        if word_count + len(word) + 1 <= max_length:
            ...
            break
    return " ".join(optimized) + "... (optimized)"