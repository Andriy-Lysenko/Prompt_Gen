...
# Генерация главного промпта с улучшенной структурой для Bedside Table и Side Table
if st.session_state.furniture_type == "Table" and (st.session_state.furniture_subtype == "Bedside Table" or st.session_state.furniture_subtype == "Side Table"):
    ...
    main_prompt += "The room has a warm and inviting atmosphere with minimal wooden elements in the background. "
    main_prompt += f"Captured in {camera_control[st.session_state.camera]} " if st.session_state.camera != "None" else ""
    main_prompt += "with hyperrealistic 3D rendering, ultra-detailed, 8K resolution, photorealistic, cinematic quality, sharp focus, lifelike lighting, realistic shadows, intricate details, crystal-clear image quality, bright lighting, vivid colors."
else:
...
# Оптимизация промпта (уменьшение до 400 символов)
...
# Применение оптимизации при нажатии кнопки "Optimize"
if st.button("Optimize", help="Optimize the prompt to fit within 400 characters"):
    main_prompt = optimize_prompt(main_prompt)
    st.rerun()
